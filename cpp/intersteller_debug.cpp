...
int main(){
    ...
    // Debug: print positions for first 10 time steps
    cerr << "Positions for first 10 time steps:" << endl;
    for(int i = 0; i < counter; i++){
        cerr << IdToName[i] << ": ";
        for(int t = 0; t <= 9; t++){
            cerr << "(" << positions[i][t].first << "," << positions[i][t].second << ") ";
        }
        cerr << endl;
    }
    ...
    // Start at the source planet at time 0
    visited[nodeId[sourcePlanet]][0] = true;
    q.push({nodeId[sourcePlanet], 0});
    
    cerr << "Starting at " << sourcePlanet << " with position (" << positions[nodeId[sourcePlanet]][0].first << "," << positions[nodeId[sourcePlanet]][0].second << ")" << endl;
    ...
    while(!q.empty()){
        ...
        cerr << "At time " << curTime << ": at planet " << IdToName[curNode] << " pos (" << positions[curNode][curTime].first << "," << positions[curNode][curTime].second << ")" << endl;
        ...
        for(int nextNode = 0; nextNode < counter; nextNode++){
            if(!visited[nextNode][nextTime]){
                if(positions[curNode][curTime] == positions[nextNode][nextTime]){
                    cerr << "  Can transfer to " << IdToName[nextNode] << " at time " << nextTime << endl;
